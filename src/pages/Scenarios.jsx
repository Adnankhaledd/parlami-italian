              ...
              <Link to={`/scenarios/${scenario.id}`}>
                <div
                  className={`card-hover relative overflow-hidden group ${
                    categoryBorderColors[scenario.category]
                  }`}
                >
                  {/* Completion badge */}
                  {completed && (
                    <div className="absolute top-3 right-3 w-6 h-6 rounded-full bg-olive flex items-center justify-center">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <span
                      className={`text-xs font-medium px-2.5 py-1 rounded-lg bg-gradient-to-r ${
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {scenario.keyVocabulary.slice(0, 4).map((word) => (
                      <span
                        key={word}
                        className="text-xs bg-navy-700/50 text-cream/60 px-2 py-0.5 rounded-md"
                      >
                        {word}
                      </span>
                    ))}
